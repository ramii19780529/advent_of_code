...
def orientations(beacons):
    ...
    """
    return {
        orientation: {orientate(beacon, *orientation) for beacon in beacons}
        for orientation in [
            (x, y, z, i, j, k)
...
def offsets(beacons):
    ...
    """
    return {
        point_one: {subtract_points(point_one, point_two) for point_two in beacons}
...
def overlap(beacons, absolute_beacons):
    ...
    for orientation, beacon_orientations in orientations(beacons).items():
        ...
        for point_one, point_two in [
            ...
            if len(absolute_beacon_offsets[point_one] & beacon_offsets[point_two]) > 11:
...
while scanners:
    ...
    result = overlap(relative_beacon_positions, absolute_beacon_positions)
